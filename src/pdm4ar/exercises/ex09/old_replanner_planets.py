...
class OldSolverParameters:
    ...
    weight_p: NDArray = field(
        default_factory=lambda: 5 * np.array([[1.0]]).reshape((1, -1))
...
class OldRocketPlanner:
    ...
    def __init__(
        ...
        # Solver Parameters
        self.params = OldSolverParameters()
    ...
    def compute_trajectory(
        ...
        self.U_bar[:] = 0
        self.p_bar[0] = 30.0  # Guess for final time
        ...
        # print("-" * 18 + "Optimal X" + "-" * 18)
        # print(self.X_new)
        # print()
        # print("-" * 18 + "Optimal U" + "-" * 18)
        # print(self.U_new)
        # print()
        # print("-" * 18 + "Optimal p" + "-" * 18)
        # print(self.p_new)
        # print()
        # print("-" * 18 + "Optimal nu" + "-" * 18)
        # print(self.variables["nu"].value)
        # print()
        slack = 0
        for j, planet in enumerate(self.planets):
            slack += cvx.sum(self.s_prime[j]).value
    ...
    def _get_variables(self) -> dict:
        ...
        for planet in self.planets:
            self.s_prime.append(cvx.Variable((self.params.K, 1), nonneg=True))
    ...
    def _get_constraints(self) -> list[cvx.constraints]:
        ...
        for j, planet in enumerate(self.planets):
            ...
            for i in range(self.variables["X"].shape[1]):
                ...
                constraints.append(1 + C_current <= self.s_prime[j])

        """
        def create_polygon_approximation(center, radius, num_sides):
            # This function generates the vertices of a polygon that approximates a circle
            theta = np.linspace(0, 2 * np.pi, num_sides, endpoint=False)
            vertices = np.array(
                [center + radius * np.array([np.cos(t), np.sin(t)]) for t in theta]
            )
            return vertices
        ...
        for planet in self.planets:
            ...
            for i in range(1, self.variables["X"].shape[1]):
                X_ref = self.X_bar[0:2, i - 1] + np.array(
                    [0.1, 0.1]
                )  # Adding small offset to remove divide by 0 error
                X_current = self.variables["X"][0:2, i]
                obstacle_constraint_norm = cvx.norm(
                    planet_radius * (X_ref - planet_center), 2
                )
                C_current = (
                    -1
                    * (planet_radius * planet_radius * (X_ref - planet_center))
                    / (obstacle_constraint_norm)
                    @ X_current
                )

                constraints.append(C_current <= 0)

            num_sides = 8
            # Get the vertices of the polygon
            vertices = create_polygon_approximation(
                planet_center, planet_radius, num_sides
            )

            # Create constraints for each edge of the polygon
            for i in range(num_sides):
                # Compute the normal vector to the polygon edge
                next_vertex = vertices[(i + 1) % num_sides]
                edge_vector = next_vertex - vertices[i]
                normal_vector = np.array([-edge_vector[1], edge_vector[0]])

                # Create a linear constraint for each time step
                for t in range(1, self.variables["X"].shape[1]):
                    # Point on the line
                    point_on_edge = vertices[i]

                    # The constraint ensures that the dot product between the normal vector
                    # and the vector from a point on the edge to the spacecraft position
                    # is greater than or equal to zero
                    constraint = normal_vector @ (
                        self.variables["X"][0:2, t] - point_on_edge
                    )
            ...
            for satellite in self.satellites:
                ...
                satellite_center_position = np.array(
                    [
                ...
                # Calculating constraint for initial position of satellite
                X_ref = self.X_bar[0:2, 0] + np.array(
                    [0.1, 0.1]
                )  # Adding small offset to remove divide by 0 error
                X_current = self.variables["X"][0:2, 1]
                obstacle_constraint_norm = cvx.norm(
                    satellite_radius * (X_ref - satellite_center_position), 2
                )
                C_current = (
                    -1
                    * (
                        satellite_radius
                        * satellite_radius
                        * (X_ref - satellite_center_position)
                    )
                    / (obstacle_constraint_norm)
                    @ X_current
                )
    ...
    def _calculate_linear_cost_constraint(self) -> float:
        ...
        for i, planet in enumerate(self.planets):
            cost += np.sum(self.s_prime[i].value)
