...
class MovingGoalSolverParameters:
    """
...
class MovingGoalRocketPlanner:
    ...
    sg: RocketGeometry
    sp: RocketParameters
    params: MovingGoalSolverParameters
    ...
    def __init__(
        ...
        # Solver Parameters
        self.params = MovingGoalSolverParameters()
    ...
    def compute_trajectory(
        self, init_state: RocketState, goal: SatelliteTarget, goal_target_time: float
    ) -> tuple[DgSampledSequence[RocketCommands], DgSampledSequence[RocketState]]:
        """
        Compute a trajectory from init_state to goal.
        """
        print("Running moving_goal_planner...")

        self.init_state = init_state
        self.goal_target_time = goal_target_time

        # take the goal state but put val of index 3,4,5 to 0
        # Assuming final_goal is your array

        self.goal_state = goal
        # self.goal_state.vx = 0
        # self.goal_state.vy = 0

        # self.goal_state = self.get_target_state_at(goal)
        ...
        self.U_bar[:] = 0
        self.p_bar[0] = self.goal_target_time  # Guess for final time
        self.t_min = self.goal_target_time - 0.5
        self.t_max = self.goal_target_time + 0.5
        ...
        for i in range(self.params.max_iterations):
            ...
            #     print(obstacle + " slack: " + str(self.s_prime[obstacle].value))
            #     slack += cvx.sum(self.s_prime[obstacle]).value
            # print("Total slack from iteration: ", slack)
        ...
        return mycmds, mystates, self.p_bar[0], i
    ...
    def _get_problem_parameters(self) -> dict:
        ...
        problem_parameters = {
            ...
            "t_min": cvx.Parameter(nonneg=True),
            "t_max": cvx.Parameter(nonneg=True),
        }
    ...
    def _get_constraints(self) -> list[cvx.constraints]:
        ...
        constraints = [
            ...
            self.variables["p"][:] >= self.problem_parameters["t_min"],
            self.variables["p"][:] <= self.problem_parameters["t_max"],
        ...
        for i, satellite in enumerate(self.satellites):
            ...
            satellite_radius = self.satellites[satellite].radius + self.sg.l / 2
    ...
    def _convexification(self):
        ...
        self.problem_parameters["t_min"].value = self.t_min
        self.problem_parameters["t_max"].value = self.t_max
    ...
    def _get_sat_center(self, satellite: FullSatelliteParams):
        ...
        for t in range(self.params.K):
            ...
            omega_t = omega * t
            approx_pt = 0.5 * omega * t_f + tau

            cos_omega_t = orbit_r * (
                np.cos(approx_pt)
                - np.sin(approx_pt) * (omega_t)
                - (np.cos(approx_pt) / 2) * omega_t**2
                # + (np.sin(tau) / 6) * omega_t**3
            )
            sin_omega_t = orbit_r * (
                np.sin(approx_pt)
                + np.cos(approx_pt) * (omega_t)
                - (np.sin(approx_pt) / 2) * omega_t**2
                # - (np.cos(tau) / 6) * omega_t**3
            )
            # Note: we fixed the derivative here
    ...
    def get_target_state_at(self, goal) -> DynObstacleState:
        """
        Retrieves the goal based on the final time
        """
        t = 30
        print("p_bar: ", t)

        goal_offset_r_factor = 2
        cos_omega_t = (goal.orbit_r + goal.offset_r * goal_offset_r_factor) * (
            np.cos(goal.tau) - np.sin(goal.tau) * (goal.omega * t)
        )
        sin_omega_t = (goal.orbit_r + goal.offset_r * goal_offset_r_factor) * (
            np.sin(goal.tau) + np.sin(goal.tau) * (goal.omega * t)
        )  # Note: that previous derivative was wrong
        x = cos_omega_t + goal.planet_x
        y = sin_omega_t + goal.planet_y

        psi = (np.pi / 2 + goal.omega * t + goal.tau) % (2 * np.pi)

        v = goal.omega * goal.orbit_r
        vx = v * (
            np.sin(psi) + np.cos(psi) * (psi * t)
        )  # Note: that derivative could be mistake
        vy = v * (np.cos(psi) - np.sin(psi) * (psi * t))

        final_goal = np.array([x, y, psi, 0, 0, 0])
        final_goal = DynObstacleState.from_array(final_goal)
